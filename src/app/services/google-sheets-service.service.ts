...
export class GoogleSheetsServiceService {
  ...
  private readonly spreadsheetId =
    '1vy4ASlXc5onreOqVWfzywkFETZhETrnEJSiJYm4eGJ0';
  ...
  constructor(private http: HttpClient) {
    let body = {
      ...
      client_id:
        '974479645657-n6ek4oik4n2b5ngjvkecs3tsl1eaed9h.apps.googleusercontent.com',
      client_secret: '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',
      refresh_token:
        '1//0gwP9bqkBYn0nCgYIARAAGBASNwF-L9IrPFNyslQMqtd4XRCBSP8VyINLnc5ws40Sbvh8seChdlSk0zk9BkjETBvhVlxLCUMSHL8',
  ...
  async sendDataToSheet(timesheet: any): Promise<void> {
    ...
    await timesheet.rows.forEach((row: any) => {
      ...
      row.labours.forEach((item: any) => {
        if (row.isStatusCheckBox) {
          row.activityType = row.statusCheckBox;
        }

        const averageTimeInHours = row.manHoursSpent / row.labours.length;

        // Calculate the hours and minutes
        const hours = Math.floor(averageTimeInHours);
        const minutes = Math.round((averageTimeInHours - hours) * 60);

        // Format hours and minutes with leading zeros
        const formattedHours = String(hours).padStart(2, '0');
        const formattedMinutes = String(minutes).padStart(2, '0');
        // console.log(typeof item.manHoursSpent); // Check the data type of item.manHoursSpent
    ...
    if (file != undefined && file != null) {
      ...
      let fileArray = [
        [
          ...
          localStorage.getItem('user'),
          timesheet.selectedImage.name,
          'https://drive.google.com/drive/folders/1V70v6xknjtCAEqo0Vwb3UbB_Dry0SU9W',
        ],
      ];
